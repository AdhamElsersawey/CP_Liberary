struct SGTBeats {
    ...
    void _update_mod(ll x, int a, int b, int k, int l, int r) {
        if (b <= l || r <= a || max_v[k] < x) {
            return;
        }

        if (a <= l && r <= b && max_v[k] == min_v[k]) {
            updateall(k, max_v[k] % x);
            return;
        }

        push(k);
        _update_mod(x, a, b, 2 * k + 1, l, (l + r) / 2);
        _update_mod(x, a, b, 2 * k + 2, (l + r) / 2, r);
        update(k);
    }

    // range mod update
    void update_mod(int a, int b, ll x) {
        _update_mod(x, a, b, 0, 0, n0);
    }

    // range minimize update
    void update_min(int a, int b, ll x) {
        _update_min(x, a, b, 0, 0, n0);
    }

    // range maximize update
